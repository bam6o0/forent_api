...
type Category struct {
	ID               int `gorm:"primary_key"` // This is the category Model PK field
	MiddleCategoryID int // Belongs To MiddleCategory
	Name             string
	NameBase         string
	NameEn           string
...
type CategoryStorage interface {
	...
	ListCategory(ctx context.Context, middleCategoryID int) []*app.Category
	OneCategory(ctx context.Context, id int, middleCategoryID int) (*app.Category, error)
}
...
// CategoryFilterByMiddleCategory is a gorm filter for a Belongs To relationship.
func CategoryFilterByMiddleCategory(middleCategoryID int, originaldb *gorm.DB) func(db *gorm.DB) *gorm.DB {

	if middleCategoryID > 0 {

		return func(db *gorm.DB) *gorm.DB {
			return db.Where("middle_category_id = ?", middleCategoryID)
