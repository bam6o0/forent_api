...
func (m *RentalDB) ListRental(ctx context.Context, offerID int) []*app.Rental {
	defer goa.MeasureSince([]string{"goa", "db", "rental", "listrental"}, time.Now())

	var native []*Rental
	var objs []*app.Rental
	err := m.Db.Scopes(RentalFilterByOffer(offerID, m.Db)).Table(m.TableName()).Find(&native).Error
...
func (m *RentalDB) OneRental(ctx context.Context, id int, offerID int) (*app.Rental, error) {
	defer goa.MeasureSince([]string{"goa", "db", "rental", "onerental"}, time.Now())

	var native Rental
	err := m.Db.Scopes(RentalFilterByOffer(offerID, m.Db)).Table(m.TableName()).Preload("Offer").Where("id = ?", id).Find(&native).Error
