...
func ListPlaceOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.PlaceController, payload *app.ListPlacePayload) (http.ResponseWriter, app.PlaceCollection) {
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "PlaceTest"), rw, req, prms)
	listCtx, _err := app.NewListPlaceContext(goaCtx, req, service)
	if _err != nil {
	...
	var mt app.PlaceCollection
	if resp != nil {
		var _ok bool
		mt, _ok = resp.(app.PlaceCollection)
		if !_ok {
			t.Fatalf("invalid response media: got variable of type %T, value %+v, expected instance of app.PlaceCollection", resp, resp)
		}
		_err = mt.Validate()
		if _err != nil {
